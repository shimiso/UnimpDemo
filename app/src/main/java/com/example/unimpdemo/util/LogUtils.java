...
public class LogUtils {
    ...
    /**
     * 初始化函数
     * <p>与{@link #getBuilder(Context context)}两者选其一</p>
     *
    ...
    public static void init(boolean logSwitch, boolean log2FileSwitch, char logFilter, String tag) {
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            dir = MainApp.getApp().getExternalCacheDir().getPath() + File.separator;
        } else {
            dir = MainApp.getApp().getCacheDir().getPath() + File.separator;
        }
     ...
     * @param msg  信息
     **/
    public synchronized static void log2File(final String type, final String tag, final String msg) {
        Date now = new Date();
