...
import com.example.unimpdemo.PolicyAgreementActivity;
...
public class ConfirmDialog extends Dialog {
    ...
    public ConfirmDialog setDialogTitle(String strTitle) {
        title = (TextView) confirmDialog.findViewById(R.id.title);
        if (title != null) {
     ...
     * @return
     */
    public ConfirmDialog setDialogMessage(CharSequence strMessage, String fullTips) {
        TextView message = (TextView) confirmDialog.findViewById(R.id.message);
        if (message != null) {
            message.setText(strMessage);
        }
        setSpanTextInfo(message, fullTips);
        return confirmDialog;
    }

    private void setSpanTextInfo(TextView textView, String fullTips) {

        SpannableStringBuilder ssb = new SpannableStringBuilder();
        ssb.append(fullTips);
        //第一个出现的位置
        final int start = fullTips.indexOf("《");
        ssb.setSpan(new ClickableSpan() {

            @Override
            public void onClick(View widget) {
                //用户服务协议点击事件
                PolicyAgreementActivity.openActivity(context, "service");
            ...
            public void updateDrawState(TextPaint ds) {
                ...
                //设置文件颜色
                ds.setColor(Color.parseColor("#1c76dc"));
                // 去掉下划线
        ...
        //最后一个出现的位置
        final int end = fullTips.lastIndexOf("《");
        ssb.setSpan(new ClickableSpan() {

            @Override
            public void onClick(View widget) {
                //隐私协议点击事件
                PolicyAgreementActivity.openActivity(context, "privacy");
            }
